...
Wind<-read.csv('https://raw.githubusercontent.com/sernumo/Modelo-aerogenerador/master/Datos_medidos/2019_data_oaxaca_Windspeed.csv')
Wind <- Wind[!is.na(Wind$PCTimeStamp),]
Wind <- Wind[!is.na(Wind$Vel..m.s.),]
...
data_avail=round((length(Wind$Vel..m.s.)/Exp_data)*100,2)
...
Oax_prod<- as.data.frame(ENRgy)
print("Producción mensual con Aerogenerador")
Oax_prod$MWh <- as.character(Oax_prod$MWh)
Energy <-sum(as.numeric(Oax_prod$MWh))

Oax_prod$MWh <- round(as.numeric(Oax_prod$MWh),2)

#######Ordenamos en decresiente#######

data3 <- Oax_prod # Replicate original data
data3$Mnth <- factor(Oax_prod$Mnth,    # Factor levels in decreasing order
